...
This document preserves the March 18, 2026 public-beta snapshot for
`zpe-robotics 0.1.0`. It is not the governing March 21 engineering authority.
Use `../proofs/ENGINEERING_BLOCKERS.md` for the current completion boundary.
...
| Searchability | primitive search works on decoded benchmark trajectories | `B3` still fails because strict bit-exact replay is not proven |
...
| hostile-path auditability | packet verification and audit-bundle surfaces in `../src/zpe_robotics/` and `../tests/` | auditability does not close the blocker surface |
...
- a closed engineering blocker surface
- broader release authorization beyond the current package surface
